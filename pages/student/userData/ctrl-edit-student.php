...
if (isset($_POST['saveImg'])) {
  ...
  if (!empty($_FILES['image']['tmp_name'])) {
    ...
    $updateImg = mysqli_query($db, "UPDATE tbl_student SET img = '$image' WHERE student_id = '$student_id'") or die(mysqli_error($db));
...
if (isset($_POST['basic_info'])) {
   ...
   if (!empty($_POST['lastname'])) {
    ...
    $getAllUsername = mysqli_query($db, "SELECT username FROM tbl_super_ad WHERE username = '$username' UNION SELECT username FROM tbl_admin WHERE username = '$username' UNION SELECT username FROM tbl_registrar WHERE username = '$username' UNION SELECT username FROM tbl_student WHERE username = '$username'") or die(mysqli_error($db));
    $check = mysqli_num_rows($getAllUsername);

    $getUser = mysqli_query($db, "SELECT admin_id AS userID FROM tbl_super_ad WHERE username = '$username' UNION SELECT ad_id AS userID FROM tbl_admin WHERE username = '$username' UNION SELECT reg_id AS userID FROM tbl_registrar WHERE username = '$username' UNION SELECT student_id AS userID FROM tbl_student WHERE username = '$username'") or die(mysqli_error($db));
    $rCheck = $getUser->fetch_array();

    if ($check == 0 || $_SESSION['userid'] == $rCheck['userID']) {
  $updateInfo = mysqli_query($db, "UPDATE tbl_student SET username='$username' WHERE student_id = '$student_id'") or die(mysqli_error($db));
